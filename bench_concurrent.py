#!/usr/bin/env python3
"""Concurrent chat completion benchmark for vLLM OpenAI API.

Measures TTFT + completion tok/s for concurrency levels (default 1,4,10).
Uses stream=true so TTFT is real first-token latency.
"""
...
def post_stream(
    ...
    # placeholder filled by caller for worker_id/concurrency
    ...
    *,
    ...
    try:
        ...
        if completion_tokens is None:
            # rough fallback: ~4 chars/token on mixed reasoning+content
...
def run_level(
    *,
    ...
    with ThreadPoolExecutor(max_workers=level) as ex:
        ...
        for fut in as_completed(futs):
            ...
            print(
                ...
                f"ttft={ttft_s}s total={total_s}s "
                f"tok/s={tps} "
    ...
    avg_ttft = f"{summary['avg_ttft_s']:.3f}" if summary["avg_ttft_s"] is not None else "n/a"
    agg = (
        f"{summary['aggregate_tok_per_s']:.1f}"
        if summary["aggregate_tok_per_s"] is not None
        else "n/a"
    )
    avg_stream = (
        f"{summary['avg_tok_per_s_per_stream']:.1f}"
        if summary["avg_tok_per_s_per_stream"] is not None
        else "n/a"
    )
    print(
        f"  summary: ok={summary['succeeded']}/{summary['requested']} "
        f"avg_ttft={avg_ttft}s "
        f"agg_tok/s={agg} "
        f"avg_stream_tok/s={avg_stream}",
        flush=True,
    )
    return summary
...
def main() -> int:
    ...
    p.add_argument("--model", default="nvidia/Qwen3.6-35B-A3B-NVFP4")
